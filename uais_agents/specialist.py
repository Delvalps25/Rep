...
from uais_core.providers import REGISTRY
...
class AgentRole(_enum.Enum):
    ...
    VERIFIER             = "verifier"
    SLEEPTIME_CONSOLIDATOR = "sleeptime_consolidator"
...
class TaskScheduler:
    def __init__(self):
        self._queue: list[tuple[float, str, dict]] = []
...
class ResultMerger:
    def merge(self, results: list[dict]) -> dict:
        ...
        outputs = []
        for r in results:
            if "output" in r: outputs.append(str(r["output"]))
            elif "result" in r: outputs.append(str(r["result"]))
...
def build_specialist_pool(hw: HardwareProfile,
    ...
    def _pick(min_vram: float, max_vram: float,
        ...
        if not candidates: return hw.model
